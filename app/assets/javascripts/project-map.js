...
google.maps.event.addDomListener(window, 'load', function() {
    map = new google.maps.Map($('.project-map').get(0), {
    ...
    /*var fader = $('<div class="project-map-fader">').click(function(e) {
     var canvas = $('.project-map');
...
/*function collectFilterOptions(page) {
 var url = 'projects';
 $('.project-filter input:checked').each(function() {
...
/*function loadProjects(url, fadeOut) {
 $('.project-list').hide();
 $('.project-list').load(url + ' #content > *', function() {
 $('.project-list').fadeIn();
 removeMarkers();
 $('.project-list .project-teaser').each(function() {
 var desc = $('<div>').append($(this).clone());
 desc.find('.project-teaser > div').each(function() {
 $(this).html($(this).html().substr(0, 140) + ' ...');
 });
 addMarker($(this).find('h3').html(), desc.html(),
 $(this).data('lat'), $(this).data('lon'));
 });
 $('.project-list .project-browser a').click(function(e) {
 e.preventDefault();
