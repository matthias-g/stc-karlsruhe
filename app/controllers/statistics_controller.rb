class StatisticsController < ApplicationController
  ...
  def sort_column(action)
    case params[:sort]
      ...
      when 'free_places'
        action.desired_team_size > 0 ? action.desired_team_size - action.volunteers.count : action.total_desired_team_size - action.aggregated_volunteers.count
      ...
      when 'occupancy'
        if action.desired_team_size > 0
          ...
          100 * action.aggregated_volunteers.count / action.total_desired_team_size
        else
          0
        end
      else
